...
class QuarkW8A8FP8MoE(QuarkMoEScheme):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # Preshuffle only when the AITER runner was selected: the Triton kernels
        # read the canonical layout, and aiter.fused_moe picks its preshuffle_on
        # kernels off the is_shuffled tag.
    ...
    def create_moe_runner(
        ...
        # AITER's per_Token fused MoE consumes per-channel weight scales, so the
        # per-tensor scheme has no equivalent there; anything else runs Triton.
        use_aiter_runner = (
            _use_aiter
        ...
        # The choice is otherwise silent, and a Triton fallback where AITER was
        # expected reads as a plain perf regression. Keyed on the message, so
        # each distinct resolution is reported once.
        print_info_once(
