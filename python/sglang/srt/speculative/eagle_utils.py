...
# Split-row top-p renorm. The pivot kernel above walks one block per row, which at
...
def _top_p_renorm_split(probs: torch.Tensor, top_ps: torch.Tensor) -> torch.Tensor:
    ...
    h0 = torch.zeros((rows, _TP_NB0), **f64)
    h1 = torch.zeros((rows, _TP_NB), device=dev, dtype=torch.float32)
    h2 = torch.zeros((rows, _TP_NB), device=dev, dtype=torch.float32)
    ...
    grid = (rows * G,)
    _top_p_hist0_kernel[grid](probs, h0, mx, vocab, G, BLOCK=2048, NB0=_TP_NB0)
    _top_p_cut0_kernel[(rows,)](
        h0, top_ps, mx, p0, a0, NB0=_TP_NB0, HB=_TP_HB, WIN=_TP_WIN
    )
    _top_p_hist_lo_kernel[grid](probs, h1, p0, vocab, G, SH=8, BLOCK=2048, NB=_TP_NB)
    _top_p_cut_lo_kernel[(rows,)](
        h1, top_ps, p0, a0, p1, a1, z, LAST=False, NB=_TP_NB
    )
    _top_p_hist_lo_kernel[grid](probs, h2, p1, vocab, G, SH=0, BLOCK=2048, NB=_TP_NB)
    _top_p_cut_lo_kernel[(rows,)](h2, top_ps, p1, a1, p2, a2, z, LAST=True, NB=_TP_NB)
    _top_p_apply_kernel[grid](probs, out, p2, z, vocab, G, BLOCK=2048)
    return out
...
def _renorm_top_k_top_p_hip(
    ...
    if probs.shape[0] <= _TP_ROW_LIMIT:
        out = _top_p_renorm_split(probs, top_ps)
    else:
        out = torch.empty_like(probs)
