...
class DeepseekV2WeightLoaderMixin:
    ...
    def _maybe_quant_glm_nextn_moe_to_ptpc(self, weights, nextn_conf: NextNConfig):
        ...
        experts in bf16. Quantizing them here pairs with the QuarkW8A8FP8MoE
        scheme that GlmMoeDsaForCausalLMNextN injects for that module.
        ...
        Returns:
            Original weights iterator when the cast is off, otherwise a
            generator emitting the FP8 weights plus their ``.weight_scale``.
        """
        ...
        expert_proj_re = re.compile(
            r"mlp\.(experts\.\d+|shared_experts)\.(gate_proj|up_proj|down_proj)\.weight$"
        )
        # e4m3fn even on MI300: QuarkW8A8FP8MoE converts to fnuz on archs that
        # need it.
        ...
        def _cast() -> Iterable[Tuple[str, torch.Tensor]]:
            for name, tensor in weights:
                ...
                # One scale per output channel, matching the "per_channel"
                # weight scheme.
                scale = weight.abs().amax(dim=-1, keepdim=True).clamp(min=1e-12)
                scale /= fp8_max
                yield name, (weight / scale).clamp(-fp8_max, fp8_max).to(
                    torch.float8_e4m3fn
                )
