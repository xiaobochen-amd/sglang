// Torch pybind shim for csrc/logits_kernel.hip (section B).
//
...
extern "C" int launch_logits(const void* q, const void* kv, const void* wgt,
                             ...
                             int batch, int heads, int max_pages, int out_stride,
                             int blocks_per_row, int warps, void* stream,
                             void* ghist, int hist_bits, int topk);
...
static void logits_hist(at::Tensor q, at::Tensor kv, at::Tensor weights,
                        ...
                        int64_t blocks_per_row, int64_t hist_bits,
                        int64_t topk, int64_t warps) {
