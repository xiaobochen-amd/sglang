// Adapted from: https://github.com/ROCm/aiter  aiter/csrc/include/coop_topk.cuh (MIT)
//
...
// Physical KV slot of a row-relative position: pt64[row][p >> 6] * 64 + (p & 63)
// for page_size 64, which is the definition of page_table_1.  PB/PM carry the
// shift and mask so page_size 1 collapses to the identity.
...
// is the Phase L interlock: it is what the logits kernel's per-block count table must
...
find_thr_hier(const uint32_t* __restrict__ gh, uint32_t want, uint32_t crs, uint32_t total_must_be) {
...
__device__ __forceinline__ float ld_val(const float* p, bool agl) {
  return agl ? __hip_atomic_load(p, __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT) : *p;
}
...
#pragma unroll
    ...
    refine_core<true, AGL>(n, above, remain, s_key, s_slot, cv, ci, o, s_hist, s_grp, s_thr, s_above, s_emit);
  } else {
    refine_core<false, AGL>(n, above, remain, s_key, s_slot, cv, ci, o, s_hist, s_grp, s_thr, s_above, s_emit);
  }
}
...
template <int PTMODE, bool FUSEREF, bool HIER = false, bool NACQ = false, bool PRANK = false, int PBLK = 0>
__global__ __launch_bounds__(BS) void k_scatter(
  ...
  if (row_len <= TOPK) {
    // Phase H1.  When the histogram is built inside the logits kernel, k_hist is
  ...
  if constexpr (HIER) {
    ...
    if (emit_on < 5u) {
      thr_h = find_thr_hier_impl<false>(gh, TOPK, pre_crs, row_len, &sbk, &hier_above);
      ...
      if (p < STAGE) {
        ...
        if (q < cap) {
          if constexpr (FUSEREF) {
            __hip_atomic_store(&ci[q], (int32_t)gi, __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT);
  ...
  // slot -- the logits kernel computed it from the page-table entry it had loaded for
...
#pragma unroll
  for (uint32_t u = 0; u < PERT; ++u) {
    ...
    if (i < wn) {
      wslot[u] = (emit_on == 4u || pre_slotted) ? s_wbuf[i] : SLOT((uint32_t)s_wbuf[i]);
    }
    if (i < cn) {
      cslot[u] = (emit_on == 4u || pre_slotted) ? s_cidx[i] : SLOT((uint32_t)s_cidx[i]);
      cvalr[u] = s_cval[i];
    }
  }
...
#pragma unroll
  ...
  if constexpr (FUSEREF) {
    ...
    if (tx == 0) {
      const uint32_t old =
          __hip_atomic_fetch_add((uint32_t*)&rc[RC_ARR], 1u, __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT);
    ...
    const uint32_t above = NACQ ? (uint32_t)__hip_atomic_load(&rc[RC_WIN], __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT)
                                : (uint32_t)rc[RC_WIN];
    const uint32_t nraw = NACQ ? (uint32_t)__hip_atomic_load(&rc[RC_CAND], __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT)
                               : (uint32_t)rc[RC_CAND];
    ...
    if (PRANK && tx == 0 && nranked != 0u) {
      __hip_atomic_fetch_add((uint32_t*)&rc[RC_DBG], 1u, __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT);
    }
    ...
    if constexpr (PRANK) {
      ...
      if (tx == 0) {
        ...
        __hip_atomic_fetch_add((uint32_t*)&rc[RC_DBG2], 1u, __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT);
        const uint32_t d =
            __hip_atomic_fetch_add((uint32_t*)&rc[RC_DEP], 1u, __ATOMIC_RELAXED, __HIP_MEMORY_SCOPE_AGENT);
        if (d + 1u == PB_N) {
          rc[RC_WIN] = 0;
          rc[RC_CAND] = 0;
          rc[RC_ARR] = 0;
...
static bool prank_resident_ok(const void* fn, int blocks_per_row, int rows) {
  ...
  int per_cu = 0;
  if (hipOccupancyMaxActiveBlocksPerMultiprocessor(&per_cu, fn, (int)dsa_topk::BS, 0) != hipSuccess) {
    return false;
  }
  if (per_cu <= 0) {
    return false;
...
void topk_transform(
  ...
  TORCH_CHECK(
      ghist.numel() == R * (int64_t)dsa_topk::GH_STRIDE,
      "ghist must be [rows, ",
      dsa_topk::GH_STRIDE,
      "], got ",
      ghist.numel());
  TORCH_CHECK(cap >= (int64_t)dsa_topk::TOPK, "candidate capacity must be >= TopK");
  TORCH_CHECK(logits.scalar_type() == at::kFloat && out.scalar_type() == at::kInt, "dtype");
  ...
  // (the logits kernel built the histogram, so k_scatter owns the short-row path),
  // dbg=7 and selfclean=0.  The candidate-stream pointers are unused here.
#define SCATTER_ARGS                                                                                            \
  grid, blk, 0, stream, logits.data_ptr<float>(), row_ends.data_ptr<int32_t>(), page_table.data_ptr<int32_t>(), \
      out.data_ptr<int32_t>(), (uint32_t*)ghist.data_ptr<int32_t>(), cursor.data_ptr<int32_t>(),                \
      cand_cnt.data_ptr<int32_t>(), cand_idx.data_ptr<int32_t>(), cand_val.data_ptr<float>(), L, PTS, PB, PM,   \
      (uint32_t)cap, G, 1u, 1u, 7u, 0u, nullptr, nullptr, nullptr
  ...
  if (prank_resident_ok((const void*)dsa_topk::k_scatter<2, true, true, true, true>, (int)G, (int)R)) {
    hipLaunchKernelGGL((dsa_topk::k_scatter<2, true, true, true, true, 16>), SCATTER_ARGS);
  } else {
    hipLaunchKernelGGL((dsa_topk::k_scatter<2, true, true, true>), SCATTER_ARGS);
  }
#undef SCATTER_ARGS
}
...
int64_t hist_stride() {
  return (int64_t)dsa_topk::GH_STRIDE;
}
